import os, os.path
from utils import exec,patch_parser
...
def generateTrampolineFunction(name,target_function,instruction):
    template = Template(TRAMPOLINE_FUNCTION)
    return template.substitute(name=name+"_trampoline", target_function=target_function,instruction=instruction)

def generateTrampolineFile(patchesList, dependencies):
    content = ""
    for patch in patchesList:
        if patch["type"] == "function_hook" and patch["dependency"] in dependencies:
            content += generateTrampolineFunction(patch["name"], patch["target_function"],patch["instruction"])
...
def generatePatchesFile(target, dependencies):
		...
		for symbol in symbols:
			...
			if section in included_sections:
				if section == "r":
					...
					out,err = exec.execute_with_output(["arm-none-eabi-objcopy", elfFile, "--dump-section", sections[section]+"="+buildDirectory+"/section.bin"])
					...
					if section == "B" or section == "b":
						...
						# dump the code for that symbol
						out,err = exec.execute_with_output(["arm-none-eabi-objcopy", elfFile, "--dump-section", sections[section]+"."+name+"="+buildDirectory+"/section.bin"])
						if b"can't dump section" not in err:
							# recover the code for that symbol
							with open(buildDirectory+"/section.bin","rb") as content:
								output_buffer = "ram,"+baseAddress+","+content.read().hex()+","+name+"\n"
	...
	if len(missed_sections) != 0:
	    if all(list(map(lambda x: ".text.__" in x[0],missed_sections))):
	        ...
	        out,err = exec.execute_with_output(["arm-none-eabi-objcopy", elfFile, "--dump-section", ".text="+buildDirectory+"/section.bin"])
	        if b"can't dump section" not in err:
	            # recover the code for that symbol
	            with open(buildDirectory+"/section.bin","rb") as content:
	...
	for patch in targetPatches:
		if patch["dependency"] in dependencies:
			if patch["type"] == "function_hook" and patch["name"]+"_trampoline" in functions:
				instruction = "bl "+functions[patch["name"]+"_trampoline"]
				instruction = asm(instruction,arch="thumb",vma=patch["address"])
