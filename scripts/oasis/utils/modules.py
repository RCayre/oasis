...
from oasis.utils import exec,builder,conf_parser
...
def getModules():
    modulesDirectory = os.path.abspath(os.path.dirname(__file__)+"/../../../modules/")
    return os.listdir(modulesDirectory)
...
def getModuleCallbacks(name,architecture="arm"):
    time_callbacks = []
    scan_callbacks = []
    conn_init_callbacks = []
    ...
    if os.path.exists(moduleBuildDirectory):
        for symbol in exec.execute([conf_parser.getNm(architecture), moduleBuildDirectory+"/module.o"]):
            if " T " in symbol:
