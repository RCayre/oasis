...
from .generators import patch_generator,linker_generator,wrapper_generator,conf_generator
from interface.internalblue import InternalblueInterface
import struct
...
class BroadcomController(Controller):
    ...
    def extractTimestampFunctions(self):
        ...
        for address in patterns.findPattern(self.firmware,pattern2):
            instructions = list(disassembler.exploreInstructions(self.instructions[address-32:address]))[::-1]
            for instrIndex in range(len(instructions)-1):
                if "bx" in instructions[instrIndex+1] and "lr" in instructions[instrIndex+1]:
                    convertTimestampInUsAddress = disassembler.extractInstructionAddress(instructions[instrIndex])
                    break
    ...
    def extractBcsulpSetupWhitening(self):
        ...
        for address in patterns.findPattern(self.firmware,pattern1):
            instructions = list(disassembler.exploreInstructions(self.instructions[address-32:address]))
            for instrIndex in range(len(instructions)-1):
                if "ldr" in instructions[instrIndex] and "ldr" in instructions[instrIndex+1]:
                    bcsulpWhiteningAddress = disassembler.extractInstructionAddress(instructions[instrIndex])
                    break
    ...
    def extractConnectionTaskFunctions(self):
        ...
        if connTaskSlotIntAddress is not None:
            ...
            if len(candidates) == 1 or (len(candidates) > 1 and len(set1) == 1 and len(set2) == 1 and len(set3) == 1):
                ...
                connTaskRxDoneAddress = disassembler.extractAddressFromFunctionPointer(self.firmware[baseConnectionTaskAddressRom+24:baseConnectionTaskAddressRom+28])
                connTaskTxDoneAddress = disassembler.extractAddressFromFunctionPointer(self.firmware[baseConnectionTaskAddressRom+28:baseConnectionTaskAddressRom+32])
                connTaskRxHeaderDoneAddress = disassembler.extractAddressFromFunctionPointer(self.firmware[baseConnectionTaskAddressRom+32:baseConnectionTaskAddressRom+36])
                connTaskSlotIntAddress = disassembler.extractAddressFromFunctionPointer(self.firmware[baseConnectionTaskAddressRom+36:baseConnectionTaskAddressRom+40])

                connTaskDeleteAddress = disassembler.extractAddressFromFunctionPointer(self.firmware[baseConnectionTaskAddressRom+8:baseConnectionTaskAddressRom+12])
    ...
    def extractScanAndInitTaskFunctions(self):
        ...
        if scanTaskLcuSoftResetAddress is not None:
            ...
            for candidate in candidates:
                potentialInitTaskSlotIntAddress = disassembler.extractAddressFromFunctionPointer(self.firmware[candidate+36:candidate+36+4])
                ...
                if len(patterns.findPattern(self.firmware[potentialInitTaskSlotIntAddress:potentialInitTaskSlotIntAddress+128],pattern2)) > 0:
                    initTaskRxDoneAddress = disassembler.extractAddressFromFunctionPointer(self.firmware[candidate+24:candidate+24+4])
                    initTaskTxDoneAddress = disassembler.extractAddressFromFunctionPointer(self.firmware[candidate+28:candidate+28+4])
                    initTaskRxHeaderDoneAddress = disassembler.extractAddressFromFunctionPointer(self.firmware[candidate+32:candidate+32+4])
                    initTaskSlotIntAddress = disassembler.extractAddressFromFunctionPointer(self.firmware[candidate+36:candidate+36+4])
                else:
                    scanTaskRxDoneAddress = disassembler.extractAddressFromFunctionPointer(self.firmware[candidate+24:candidate+24+4])
                    scanTaskTxDoneAddress = disassembler.extractAddressFromFunctionPointer(self.firmware[candidate+28:candidate+28+4])
                    scanTaskRxHeaderDoneAddress = disassembler.extractAddressFromFunctionPointer(self.firmware[candidate+32:candidate+32+4])
                    scanTaskSlotIntAddress = disassembler.extractAddressFromFunctionPointer(self.firmware[candidate+36:candidate+36+4])
    ...
    def extractScanTaskRxPacketUpdateFunction(self):
        ...
        for address in patterns.findPattern(self.firmware,pattern):
            instructions = list(disassembler.exploreInstructions(self.instructions[address-32:address+32]))
            for instrIndex in range(len(instructions)-2):
                if "push" in instructions[instrIndex] and "mov" in instructions[instrIndex+1] and "bl" in instructions[instrIndex+2] and "#3, #1" in "\n".join(instructions[instrIndex+2:]):
                    scanTaskRxPktUpdateAddress = disassembler.extractInstructionAddress(instructions[instrIndex])
    ...
    def extractHciFunctions(self):
        ...
        if self.firmwareInformations["hci_implementation"] == "new":
            ...
            for address in patterns.findPattern(self.firmware,patterns.generateAlternatives(pattern1,pattern2)):
                instructions = list(disassembler.exploreInstructions(self.instructions[address+4:address+50]))

                # We extract the corresponding addresses from the next instructions
                for instruction in instructions:
                    if " bl " in instruction:
                        candidatesAllocateEventAndFillHeaderAddresses.append(disassembler.extractTargetAddressFromJump(instruction))
                    elif " b." in instruction:
                        candidatesAttemptToEnqueueEventToTransportAddresses.append(disassembler.extractTargetAddressFromJump(instruction))
    ...
    def extractAdvTaskProgHw(self):
        ...
        for address in patterns.findPattern(self.firmware, pattern):
            ...
            for instruction in list(disassembler.exploreInstructions(self.instructions[address-512:address]))[::-1]:

                if "push" in instruction or "stmdb" in instruction:
                    advTaskProgHwAddress = disassembler.extractInstructionAddress(instruction)
                    break
    ...
    def extractOsapiWaitEvent(self):
        ...
        for address in patterns.findPattern(self.firmware, patterns.generateAlternatives(pattern1,pattern2,pattern3)):
            for instruction in list(disassembler.exploreInstructions(self.instructions[address-512:address]))[::-1]:
                ...
                if "push" in instruction or "stmdb" in instruction:
                    osapiWaitEventAddress = disassembler.extractInstructionAddress(instruction)
                    break
    ...
    def extractGetRawRssi(self):
        ...
        for address in patterns.findPattern(self.firmware,pattern1):
            instructions = list(disassembler.exploreInstructions(self.instructions[address-16:address]))[::-1]
            for instrIndex in range(len(instructions)-1):
                if "ldmia" in instructions[instrIndex+1] or "b." in instructions[instrIndex+1]:
                    getRawRssiAddress = disassembler.extractInstructionAddress(instructions[instrIndex])
                    break
        ...
        if getRawRssiAddress is None:
            ...
            instructions = list(disassembler.exploreInstructions(self.instructions[self.functions["connTaskRxDone"]:self.functions["connTaskRxDone"]+128]))
            for instrIndex in range(len(instructions)-2):
                if "bl" in instructions[instrIndex] and "strb" in instructions[instrIndex+1]:
                   getRawRssiAddress = disassembler.extractTargetAddressFromJump(instructions[instrIndex])
    ...
    def extractHciCallbacksTable(self):
        ...
        if self.firmwareInformations["hci_implementation"] == "new":
            ...
            for address in patterns.findPattern(self.firmware,pattern):
                instructions = list(disassembler.exploreInstructions(self.instructions[address:address+16]))
                if "ldr" in instructions[-1]:
                    pointerToCallbackTable = disassembler.extractTargetAddressFromLoadOrStore(instructions[-1])
                    callbackTableAddress = disassembler.extractValue(self.firmware[pointerToCallbackTable:pointerToCallbackTable+4])
    ...
    def extractBdAddress(self):
        ...
        for i in patterns.findPattern(self.firmware,pattern1):
            address = disassembler.extractTargetAddressFromLoadOrStore(self.instructions[i+2])
            bdAddressPointer = disassembler.extractValue(self.firmware[address:address+4])
            break
        # Second case
        if bdAddressPointer is None:
            ...
            for i in patterns.findPattern(self.firmware,pattern2):
                address = disassembler.extractTargetAddressFromLoadOrStore(self.instructions[i+6])
        ...
        if bdAddressPointer is None:
            ...
            for i in patterns.findPattern(self.firmware,pattern3):
                address = disassembler.extractTargetAddressFromLoadOrStore(self.instructions[i+24])
                bdAddressPointer = disassembler.extractValue(self.firmware[address:address+4])
    ...
    def extractStatusRegister(self):
        ...
        instructions = list(disassembler.exploreInstructions(self.instructions[self.functions["scanTaskRxPktUpdate"]:self.functions["scanTaskRxPktUpdate"]+128]))
        ...
        if loadingStatusRegisterInstructions is not None and len(loadingStatusRegisterInstructions) == 2:
            if "ldr" in loadingStatusRegisterInstructions[1]:
                if "ldr" in loadingStatusRegisterInstructions[0]:
                    pointerAddress = disassembler.extractTargetAddressFromLoadOrStore(loadingStatusRegisterInstructions[0])
                    value = disassembler.extractValue(self.firmware[pointerAddress:pointerAddress+4])
                    offsetValue = disassembler.extractOffsetFromLoadOrStore(loadingStatusRegisterInstructions[1])

                elif "mov.w" in loadingStatusRegisterInstructions[0]:
                    value = disassembler.extractOffsetFromLoadOrStore(loadingStatusRegisterInstructions[0])
                    offsetValue = disassembler.extractOffsetFromLoadOrStore(loadingStatusRegisterInstructions[1])
    ...
    def extractRxHeaderRegister(self):
        ...
        instructions = list(disassembler.exploreInstructions(self.instructions[self.functions["initTaskRxHeaderDone"]:self.functions["initTaskRxHeaderDone"]+32]))
        ...
        for instrIndex in range(len(instructions)):
            if "and" in instructions[instrIndex] and "ldr" in instructions[instrIndex-1]:
                baseRegister = disassembler.extractBaseRegisterFromLoadOrStore(instructions[instrIndex-1])
                offset = disassembler.extractOffsetFromLoadOrStore(instructions[instrIndex-1])
                substract = 0
                for i in range(instrIndex-2,0,-1):
                    if baseRegister in instructions[i] and "subs" in instructions[i]:
                        substract = disassembler.extractOffsetFromAddOrSub(instructions[i])
                    if baseRegister in instructions[i] and "ldr" in instructions[i] and "pc" in instructions[i]:
                        address = disassembler.extractTargetAddressFromLoadOrStore(instructions[i])
                        rxHeaderRegister = disassembler.extractValue(self.firmware[address:address+4]) - substract + offset
                        break
    ...
    def extractChannel(self):
        ...
        # First method
        for address in patterns.findPattern(self.firmware,pattern1):
            instructions = list(disassembler.exploreInstructions(self.instructions[address+2:address+10]))
            if "ldr" in instructions[0] and "pc" in instructions[0] and "strb.w" in instructions[1] and "r1" in instructions[1] and "b" in instructions[2]:
                basePointer = disassembler.extractTargetAddressFromLoadOrStore(instructions[0])

                offset = disassembler.extractOffsetFromLoadOrStore(instructions[1])
                channelPointer = disassembler.extractValue(self.firmware[basePointer:basePointer+4]) + offset + 3

        # Second method
        if channelPointer is None:
            for address in patterns.findPattern(self.firmware,pattern2):
                instructions = list(disassembler.exploreInstructions(self.instructions[address+4:address+16]))

                if "bl" in instructions[0] and "ldr" in instructions[1] and "strb.w" in instructions[2]:
                    basePointer = disassembler.extractTargetAddressFromLoadOrStore(instructions[1])
                    offset = disassembler.extractOffsetFromLoadOrStore(instructions[2])
                    channelPointer = disassembler.extractValue(self.firmware[basePointer:basePointer+4]) + offset + 3

        # Third method
        if channelPointer is None:
            for address in patterns.findPattern(self.firmware,pattern3):
                instructions = list(disassembler.exploreInstructions(self.instructions[address+2:address+6]))
                basePointer = disassembler.extractTargetAddressFromLoadOrStore(instructions[0])
                scanTaskStorage = disassembler.extractValue(self.firmware[basePointer:basePointer+4])
    ...
    def extractRxRegister(self):
        ...
        for address in patterns.findPattern(self.firmware,pattern):
            ...
            for instruction in instructions:
                if "ldr " in instruction and ";" in instruction:
                    addressPointer = disassembler.extractTargetAddressFromLoadOrStore(instruction)
                    rxRegister = disassembler.extractValue(self.firmware[addressPointer:addressPointer+4])
                    break
        ...
        if rxRegister is None:
            ...
            instructions = list(disassembler.exploreInstructions(self.instructions[self.functions["scanTaskRxDone"]:self.functions["scanTaskRxDone"]+2048]))
    ...
    def extractOffsetsInConnectionStructure(self):
        ...
        for i in patterns.findPattern(self.firmware,pattern1):
            instructions = list(disassembler.exploreInstructions(self.instructions[i-64:i+64]))

            for instruction in instructions:
                if "bl" in instruction and hex(self.functions["bcsulp_setupWhitening"])[2:] in instruction:
                    instructions2 = list(disassembler.exploreInstructions(self.instructions[i-16:i+4]))
                    instructions3 = list(disassembler.exploreInstructions(self.instructions[i-16:i+16]))

                    destRegister = disassembler.extractDestRegisterFromBic(instructions2[-1])
                    initRegister = None
                    ...
                    for instruction in instructions2[::-1]:
                        ...
                        if "ldr" in instruction and "#0" in instruction and destRegister+"," in instruction and initRegister is None:
                            initRegister = disassembler.extractBaseRegisterFromLoadOrStore(instruction)

                        if "ldr" in instruction and ";" in instruction and initRegister is not None and initRegister+"," in instruction:
                            offsetSecondStructure = disassembler.extractOffsetFromLoadOrStore(instruction)
                            break
                    for instrIndex in range(len(instructions3)-2):
                        if ("ldr" in instructions3[instrIndex] and "bic" in instructions3[instrIndex+1]) or  ("ldr" in instructions3[instrIndex] and "add" in instructions3[instrIndex+1] and "bic" in instructions3[instrIndex+2]):
                            crcInitOffsetInSecondStructure = disassembler.extractOffsetFromLoadOrStore(instructions3[instrIndex])
                            break
            for instrIndex in range(len(instructions)-2):
                if "ldrb.w" in instructions[instrIndex] and ";" in instructions[instrIndex] and "bl" in instructions[instrIndex+1] and hex(self.functions["bcsulp_setupWhitening"])[2:] in instructions[instrIndex+1]:
                    channelOffsetInFirstStructure = disassembler.extractOffsetFromLoadOrStore(instructions[instrIndex])
        ...
        for address in patterns.findPattern(self.firmware,pattern2):
            instructions = list(disassembler.exploreInstructions(self.instructions[address:address+64]))
            ...
            for instrIndex in range(len(instructions)):
                if "bl" in instructions[instrIndex] and hex(self.functions["lculp_createAccessAddress"])[2:] in instructions[instrIndex] and "str" in instructions[instrIndex+1] and ";" in instructions[instrIndex+1]:
                    accessAddressOffsetInSecondStructure = disassembler.extractOffsetFromLoadOrStore(instructions[instrIndex+1])
                    break

            instructions = list(disassembler.exploreInstructions(self.instructions[address-64:address]))[::-1]
            for instrIndex in range(len(instructions)-2):
                if "bl" in instructions[instrIndex] and "add" in instructions[instrIndex+1]:
                    channelMapOffsetInSecondStructure = disassembler.extractOffsetFromAddOrSub(instructions[instrIndex+1])
                    break
                if "strb" in instructions[instrIndex] and "str" in instructions[instrIndex+2]:
                    channelMapOffsetInSecondStructure = disassembler.extractOffsetFromLoadOrStore(instructions[instrIndex+2])
                    break

        instructions = list(disassembler.exploreInstructions(self.instructions[self.functions["connTaskRxHeaderDone"]:self.functions["connTaskRxHeaderDone"]+128]))

        for instrIndex in range(len(instructions)-1):
            if "ldrb " in instructions[instrIndex] and "cmp" in instructions[instrIndex+1] and "#1" in instructions[instrIndex+1]:
                isSlaveOffsetInFirstStructure = disassembler.extractOffsetFromLoadOrStore(instructions[instrIndex])
                break

        instructions = list(disassembler.exploreInstructions(self.instructions[self.functions["connTaskSlotInt"]:self.functions["connTaskSlotInt"]+256]))
        for instrIndex in range(len(instructions)-1):
            if "ldrh " in instructions[instrIndex]:
                hopIntervalOffsetInSecondStructure = disassembler.extractOffsetFromLoadOrStore(instructions[instrIndex])
                break
