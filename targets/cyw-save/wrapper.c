...
uint8_t* bthci_event_AllocateEventAndFillHeader (uint8_t len_total, char event_code, uint8_t len_data);
...
void on_event_loop() {
    ...
    lock = 1;
    uint32_t time = now();
    if (time - last_timestamp_in_event_loop  > 1000000) {
        last_timestamp_in_event_loop = time;
        process_time();
    }
    lock = 0;
  }
