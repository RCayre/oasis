...
class SoftDeviceController(Controller):
    ...
    def extractSetChannel(self):
        ...
        for i in patterns.findPattern(self.firmware,pattern):
            instructions = list(disassembler.exploreInstructions(self.instructions[i:i+32]))
    ...
    def extractSetAccessAddress(self):
        ...
        for i in patterns.findPattern(self.firmware,pattern):
            instructions = list(disassembler.exploreInstructions(self.instructions[i-16:i]))[::-1]
            if "ldrb" in "\n".join(instructions):
                for j in range(len(instructions)):
                    if "bx" in instructions[j] and "lr" in instructions[j]:
                        setAccessAddressAddress  = disassembler.extractInstructionAddress(instructions[j-1])
